...
public class VistaAgenda extends MouseAdapter implements ActionListener {
	...
	JTable tablaListado;
	JButton botónAñadir, botónBorrar, botónCargarTodo, botónSalvar, botónEditar;
	...
	protected void construyeVentanaAgenda() {
		...
		botónCargarTodo = new JButton("Cargar Todo");
		
		botónBuscar = new JButton("Buscar");
	    botónBuscar.setMnemonic('S');
	    panelInferior.add(botónBuscar);
		...
		panelInferior.add(botónCargarTodo); 
		botónCargarTodo.addActionListener(this);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		switch(arg0.getActionCommand()) {
			...
			case txtCargarSerial : 
				...
				break;
			case "Cargar Todo":
			    List<Contacto> listaContactos = controlador.cargarContactosDesdeBD();
			    actualizaListado(listaContactos);
			    break;
			    
			case "Buscar":
	            // Mostrar un cuadro de diálogo para que el usuario ingrese el nombre a buscar
	            String nombreBuscado = JOptionPane.showInputDialog(ventana, "Introduce el nombre del contacto:", "Buscar Contacto", JOptionPane.PLAIN_MESSAGE);

	            // Solo proceder si el usuario ingresó algo y no canceló
	            if (nombreBuscado != null && !nombreBuscado.trim().isEmpty()) {
	                Contacto contacto = controlador.buscarContactoPorNombre(nombreBuscado.trim());
	                if (contacto != null) {
	                    List<Contacto> resultado = List.of(contacto); // Para actualizar la tabla con un solo contacto
	                    actualizaListado(resultado); // Refrescar la tabla con el contacto encontrado
	                } else {
	                    JOptionPane.showMessageDialog(ventana, "No se encontró el contacto", "Resultado de búsqueda", JOptionPane.INFORMATION_MESSAGE);
	                }
	            } else {
	                JOptionPane.showMessageDialog(ventana, "Nombre no válido o búsqueda cancelada", "Error", JOptionPane.WARNING_MESSAGE);
	            }
	            break;
		}
	} // actionPerformed
